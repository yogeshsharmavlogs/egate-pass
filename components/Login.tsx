...
export default function Login({ navigation }: any) {
  ...
  const [errors, setErrors] = useState<{ mobile?: string; mpin?: string }>({});

  const validate = () => {
    let valid = true;
    let errs: { mobile?: string; mpin?: string } = {};
    ...
    >
      ...
      >
        ...
        <View className="mx-4 mt-8 rounded-2xl bg-white p-6 shadow">
          ...
          <TextInput
            className="mb-1 rounded-lg border border-[#E5E7EB] bg-[#F7F9FB] px-4 py-3"
          ...
          />
          {errors.mobile ? (
            <Text className="text-xs text-red-500 mb-2">{errors.mobile}</Text>
          ) : null}

          <Text className="mb-1 text-[#222]">MPIN</Text>
          <View className="mb-1 flex-row items-center rounded-lg border border-[#E5E7EB] bg-[#F7F9FB] px-4">
            <TextInput
          ...
          {errors.mpin ? (
            <Text className="text-xs text-red-500 mb-2">{errors.mpin}</Text>
          ) : null}
          ...
          <View className="mb-6 flex-row items-center justify-between">
            ...
            <TouchableOpacity>
              <Text className="text-xs font-semibold text-[#FF7A1A]">
                Forgot MPIN?
              </Text>
            </TouchableOpacity>
          </View>
          ...
          <TouchableOpacity
            className="mb-2 rounded-lg bg-[#FF7A1A] py-3 active:opacity-80"
