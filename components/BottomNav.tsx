...
export default function BottomNav({ active = "Dashboard", navigation }: any) {
  return (
    <View className="absolute left-0 right-0 bottom-0 pb-2 flex-row bg-white border-t border-gray-200 h-20 items-center justify-around z-50">
      ...
      <TouchableOpacity className="items-center flex-1" onPress={() => navigation.navigate('UpdateSecretary')}>
        <Ionicons name="person-add-outline" size={24} color={active === "UpdateSecretary" ? "#FF7A1A" : "#A0AEC0"} />
     <Text className={`text-xs mt-1 font-semibold ${active === "UpdateSecretary" ? "text-[#FF7A1A]" : "text-gray-400"}`}>Update Sec</Text>
      </TouchableOpacity>
